...
export const GhostCursor: React.FC<GhostCursorProps> = ({
  ...
  const pixelBudget = targetPixels ?? (isTouch ? 1.0e6 : 4.0e6);
  ...
  const fragmentShader = `
    ...
    vec4 blob(vec2 p, vec2 mousePos, float intensity, float activity) {
      ...
      float alpha = pow(smoke, 4.2) * distFactor;
  ...
  function calculateScale(el: HTMLElement) {
    // Return a fixed scale to keep it minimal and consistent across all screen sizes
    return 0.8;
  }
